...
TEXT FILE OF K&R, AS A C FILE SO SNIPPETS GET SYNTAX HIGHLIGHTING

QUOTES OF FORM		\*	...	*\	TO CONTROL QUOTES IN THE TEXT THAT UNINTENTIONALLY COMMENT OUT UNDESIRED SECTIONS
...
g,G         precision; otherwise use %f. Trailing zeros and a trailing decimal point are not
            printed.
...
                                                                                                           183
                                                                          E2
E2 bit positions. The right shift is equivalent to division by 2 if E1 is unsigned or it has a
...
__STDC__
           The constant 1. It is intended that this identifier be defined to be 1 only in standard-
...
frexp(x, int
*ip)             and a power of 2, which is stored in *exp. If x is zero, both parts of the
                 result are zero.
modf(x,          splits x into integral and fractional parts, each with the same sign as x. It
double *ip)      stores the integral part in *ip, and returns the fractional part.
fmod(x,y)
